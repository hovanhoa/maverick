...
func TestAccountCRUD(t *testing.T) {
	...
	updated, err := database.UpdateAccount(ctx, account.ID, &newEmail, nil, nil, nil)
	require.NoError(t, err)
	...
	updated2, err := database.UpdateAccount(ctx, account.ID, nil, &newUsername, nil, nil)
	require.NoError(t, err)
...
func TestUpdateAccount_ValidationError(t *testing.T) {
	...
	_, err := database.UpdateAccount(ctx, "any_id", nil, nil, nil, nil)
	require.Error(t, err)
...
func TestUpdateAccount_NotFound(t *testing.T) {
	...
	_, err := database.UpdateAccount(ctx, "account_missing", &email, nil, nil, nil)
	require.Error(t, err)
