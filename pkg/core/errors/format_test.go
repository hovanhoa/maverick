...
func testFormatRegexp(t *testing.T, n int, arg interface{}, format, want string) {
	...
	for i, w := range wantLines {
		...
		assert.True(t, match, "test %d: line %d: fmt.Sprintf(%q, err):\n got: %q\nwant: %q", n+1, i+1, format, got, want)
	}
}
...
func testFormatCompleteCompare(t *testing.T, n int, arg interface{}, format string, want []string, detectStackBoundaries bool) {
	...
	require.Equal(t, len(want), len(got), "test %d: fmt.Sprintf(%s, err) -> wrong number of blocks: got(%d) want(%d)\n got: %s\nwant: %s\ngotStr: %q",
	...
	for i := range got {
		if strings.ContainsAny(want[i], "\n") {
			...
			require.NoError(t, err)
			require.True(t, match, "test %d: block %d: fmt.Sprintf(%q, err):\ngot:\n%q\nwant:\n%q\nall-got:\n%s\nall-want:\n%s\n",
				n+1, i+1, format, got[i], want[i], prettyBlocks(got), prettyBlocks(want))
		} else {
			// Match as message
			require.Equal(t, want[i], got[i], "test %d: fmt.Sprintf(%s, err) at block %d got != want:\n got: %q\nwant: %q", n+1, format, i+1, got[i], want[i])
		}
	}
}
